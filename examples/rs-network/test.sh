...
# Simple Reed-Solomon Network Test
...
# Build if needed
if [ ! -f "./rs-network" ]; then
    log "Building Reed-Solomon network..."
    go build
fi

log "Testing Reed-Solomon network with 3 nodes..."

# Start first node (hub) - keep alive with delayed quit
log "Starting node1 (hub)..."
(sleep 20; echo "quit") | ./rs-network -l 8001 -id node1 > node1.log 2>&1 &
PID1=$!
...
log "Starting node2..."
(sleep 20; echo "quit") | ./rs-network -l 8002 -c 127.0.0.1:8001 -id node2 > node2.log 2>&1 &
PID2=$!
...
log "Starting node3..."
(sleep 20; echo "quit") | ./rs-network -l 8003 -c 127.0.0.1:8001,127.0.0.1:8002 -id node3 > node3.log 2>&1 &
PID3=$!
...
if kill -0 $PID1 $PID2 $PID3 2>/dev/null; then
    ...
    if grep -q "Using binary field GF(2\^8)" node1.log; then
        success_count=$((success_count + 1))
    fi
    if grep -q "Using binary field GF(2\^8)" node2.log; then
        success_count=$((success_count + 1))
    fi
    if grep -q "Using binary field GF(2\^8)" node3.log; then
        success_count=$((success_count + 1))
    fi

    if [ $success_count -eq 3 ]; then
        success "All 3 nodes initialized with correct binary field"
    fi
    ...
    if [ $connections_found -ne 3 ]; then
        error "Reed-Solomon network test FAILED - not all connections established ($connections_found/3)"
        log "Node1 log:"
    ...
    success "Reed-Solomon network test PASSED - all 3 connections established successfully"
else
    error "Reed-Solomon network test FAILED - some nodes crashed"
    log "Node1 log:"
    cat node1.log 2>/dev/null || true
    log "Node2 log:"
    cat node2.log 2>/dev/null || true
...
# Send test message using a background process
{
    ...
    sleep 1
    echo "quit"
} | timeout 10s ./rs-network -l 8010 -c 127.0.0.1:8001 -id test-sender > test-sender.log 2>&1 &
TEST_PID=$!
